...
const Add = () => {
  ...
  const pickImage = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      mediaType: ImagePicker.MediaTypeOptions.Images, //tipo de media disponibilizada, vamos mexer só com img
      allowsEditing: true,
      aspect: (1, 1),
      quality: 1,
    });
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={{ flex: 1, flexDirection: 'row' }}>
        <Camera ref={ref => setCamera(ref)} style={{ flex: 1, aspectRatio: 1 }} type={type} ration={'1:1'} />
      </View>
      <Button
          title='Flip Camera'
          onPress={() => {
